...
/*
  ...
  Plugin URI: http://37.57.92.40/salon/careem/
  Description: Updates user rating based on number of posts.
  Version: 1.2
  Author: Grishin Vladislav
  Author URI: http://37.57.92.40/salon/careem/
 */
require_once __DIR__ . '/../sms/expert_sms.php';
...
function registration_form( $fullname, $password, $password_confirm, $email, $website, $brand, $carnumber, $phonenumber,$code ) {
    ...
    $current_url='http'.(empty($_SERVER['HTTPS'])?'':'s').'://'.$_SERVER['SERVER_NAME'].'/salon/login-logout/';
    echo '
    <style>
	div {
		margin-bottom:2px;
	...
	<center><input type="text" name="nickname" style="width: 45%" value="' . ( isset( $_POST['nickname'] ) ? $phonenumber : null ) . '">
	...
	<button><a href="' . get_site_url() . '/login-logout/" >صفحة تسجيل الدخول</a></button>
	</div>
	</form>
	';
...
function registration_validation( $fullname, $password, $password_confirm, $email, $brand, $carnumber, $phonenumber, $website, $code) {
    ...
    if ( strlen( $fullname ) < 2 && strlen( $fullname ) > 100) {
        $reg_errors->add( 'username_length', 'Username too short. At least 2 characters is required' );
    }
    ...
    if(!preg_match("/[0-9]$/i",$phonenumber) && strlen ($phonenumber) == 14){
        $reg_errors->add( 'phonenumber', 'Incorrect phone number' );
    }
    ...
    {
        $reg_errors->add( 'brand', 'Incorrect brand name' );
    }

    if( strlen( $carnumber ) < 2 && strlen( $brand ) > 100)
    {
        $reg_errors->add( 'carnumber', 'Incorrect car number' );
    }
...
function complete_registration() {
    ...
    if ( count( $reg_errors->get_error_messages() ) < 1 ) {
        $userdata      = array(
            ...
            'first_name'  => $brand,
            'last_name'   => $carnumber,
            'nickname'    => '+38'.$phonenumber,
            'description' => get_the_title()
        );
        $expertTexting = new expert_sms();
        $from          = '380994595515';
        $to            = $userdata['nickname'];
        $msgtext       = 'Zhenyaaa awdaw';
        $user          = wp_insert_user( $userdata );
        //$expertTexting->send( $from, $to, $msgtext );
        $current_url='http'.(empty($_SERVER['HTTPS'])?'':'s').'://'.$_SERVER['SERVER_NAME'].'/salon/login-logout/';
        echo
        '<div id="overlay">
        ...
        <h2>!Registration complete</h2>
         ...
         </br>Please wait our authorization, your account will be active in 24 hours
         </p>
         </div>
         <div class="pl-right raz" >
