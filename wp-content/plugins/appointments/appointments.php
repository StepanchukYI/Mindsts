...
class Appointments {
	...
	function __construct() {
		...
		$this->gcal_image = '<img src="' . $this->plugin_url . '/images/gc_button1.gif" style="height: 50px; width: 50px"/>';
			...
			if ( $result->user ) {
				...
				if ( $userdata ) {
					...
					$name = '<a href="' . apply_filters('app_get_client_name-href', $href, $app_id, $result) . '" target="_blank">' .
						($result->name && !(defined('APP_USE_LEGACY_ADMIN_USERDATA_OVERRIDES') && APP_USE_LEGACY_ADMIN_USERDATA_OVERRIDES) ? $userdata->user_login : $userdata->user_login) .
					'</a>';
				}
				else
