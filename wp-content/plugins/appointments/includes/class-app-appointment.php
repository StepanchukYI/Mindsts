...
function appointments_insert_appointment( $args ) {
	...
	if ( $user = get_userdata( $args['user'] ) ) {
		$user = get_userdata( $args['user'] );
		$insert['user'] = $user->user_login;
		$insert_wildcards[] = '%d';
	}
	else {
		$insert['user'] = 0;
	...
	$service = appointments_get_service( $_COOKIE['abra_kadabra_cooka'] );
...
unset($_COOKIE['abra_kadabra_cooka']);
