...
//		}
	...
	function pre_confirmation () {
		...
		$service = '<label><span>' . __('اسم الخدمة', 'appointments') .  '</span>'. apply_filters('app_confirmation_service', stripslashes($service_obj->name), $service_obj->name) . '</label>';
		$start = '<label><span>' . __('التاريخ و الوقت', 'appointments') . '</span>'. apply_filters('app_confirmation_start', date_i18n($appointments->datetime_format, $start), $start) . '</label>';
		$end = '<label><span>' . __('تستمر (تقريبا)', 'appointments') . '</span>'. apply_filters('app_confirmation_lasts', $service_obj->duration . " " . __('الدقائق', 'appointments'), $service_obj->duration) . '</label>';
