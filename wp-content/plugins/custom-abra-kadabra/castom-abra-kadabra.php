...
function castom_table_abra_kadabra_ajax() {
	...
	foreach ( $services as $s ) {
		...
		$total_time  += $s['time'];
		$total_price += $price;
		array_push( $data['data'], array( $name, $s['time'], $price, $s['service_id'], ) );
	}
...
<script type="text/javascript" src="//cdn.datatables.net/1.10.15/js/jquery.dataTables.min.js"></script>';
    ...
    <script>
        ...
        var kadabra_ajax_url = '/wp-admin/admin-ajax.php';
        ...
        jQuery(document).on("change", "#optionCategory", function () {
            ...
            jQuery.post(kadabra_ajax_url, data, function (response) {
                if(response!= 0){
                    ...
                    jQuery("#error").html("الرجاء تسيجيل الدخول </br>  <a href='http://mindsts.com/login-logout/' >لدي حساب</a>");
                    jQuery("#kadabra_results").hide();
                }
...
function complete_abra_kadabra( $name, $duration, $price ) {
	...
	global $name, $duration, $price, $services_id;
	...
	//setcookie( 'abra_kadabra_cooka', $services_id, time() + 1209600 );
	$current_url = 'http' . ( empty( $_SERVER['HTTPS'] ) ? '' : 's' ) . '://' . $_SERVER['SERVER_NAME'] . '/calendar/';
