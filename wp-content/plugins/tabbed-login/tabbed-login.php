...
?>
	...
	<?php global $user_ID, $user_identity,$current_url ;
		  ...
		  $redirect_url='http'.(empty($_SERVER['HTTPS'])?'':'s').'://'.$_SERVER['SERVER_NAME'];
	get_currentuserinfo(); if (!$user_ID) { ?>

<div id="login-register-password">
	<ul class="tabs_login">
		<li class="active_login"><a href="#login" ><?php _e('تسجيل الدخول', 'tabbed-login'); ?></a></li>
		<?php  if(get_option('users_can_register')) { ?>
		<!--li><a href="#register"><?php _e('Register', 'tabbed-login') ?></a></li-->
		<?php }; ?>
		<li><a href="#forgot_password"><?php _e('هل نسيت كلمة المرور', 'tabbed-login'); ?></a></li>
	</ul>
	<div class="tab_container_login">
		<div id="login" class="tab_content_login">
			...
			<h3><?php _e('هل لديك حساب', 'tabbed-login'); ?></h3>
			...
			<form method="post" action="<?php bloginfo('url') ?>/wp-login.php" class="wp-user-form">
				...
				<div class="username">
					<label for="user_login"><?php _e('الايميل', 'tabbed-login'); ?></label>
					<input type="text" name="log" value="<?php echo esc_attr(stripslashes($user_login)); ?>" size="20" id="user_login" tabindex="11" />
				</div>
				<div class="password">
					<label for="user_pass"><?php _e('كلمه السر', 'tabbed-login'); ?></label>
					<input type="password" name="pwd" value="" size="20" id="user_pass" tabindex="12" />
				</div>
				<div class="login_fields">
					<div class="rememberme">
						<label for="rememberme" style="float: right;">
							<input type="checkbox" name="rememberme" value="forever" checked="checked" id="rememberme" tabindex="13" /><?php _e(' تذكرنى', 'tabbed-login'); ?>
						</label>
					</div>
					<?php do_action('login_form'); ?>
					<center><input type="submit" name="user-submit" value="<?php _e('دخول ', 'tabbed-login'); ?>" tabindex="14" class="user-submit" /></center>
					<input type="hidden" name="redirect_to" value="<?php if(!is_user_logged_in(wp_get_current_user()))echo $redirect_url."/login-logout/"; else echo $redirect_url."/services/";?>" />
		...
		<div id="forgot_password" class="tab_content_login" style="display:none;">
			<h3><?php _e('فقدت كلمة المرور الخاصة بك', 'tabbed-login'); ?></h3>
			<p><?php _e('أدخل اسم المستخدم الخاص بك أو البريد الإلكتروني لإعادة تعيين كلمة المرور الخاصة بك', 'tabbed-login'); ?></p>
			<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" class="wp-user-form">
				<div class="username">
					<label for="user_login" class="hide"><?php _e('اسم المستخدم أو البريد الالكتروني', 'tabbed-login'); ?></label>
					<input type="text" name="user_login" value="" size="20" id="user_login" tabindex="1001" />
				</div>
				<div class="login_fields">
					<?php do_action('login_form', 'resetpass'); ?>
					<center><input type="submit" name="user-submit" value="<?php _e('إعادة تعيين كلمة المرور', 'tabbed-login'); ?>" class="user-submit" tabindex="1002" /></center>
...
<div id="login-register-password" class="logged-in">
	<div class="sidebox">
		<h3><?php $name = get_userdata(get_current_user_id()); _e('أهلا بك,'.$name->user_login); ?> </h3>
		...
		<div class="userinfo">
			<p>
				<a href="<?php echo wp_logout_url($redirect_url."/contact/"); ?>"><?php _e('الخروج', 'tabbed-login'); ?></a>
			</p>
		</div>
	</div>
